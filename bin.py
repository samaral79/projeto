...
def encode_bin():
    ...
    for byte in bytes_texto:
        ...
        # format(byte, '08b') formata para 8 dígitos binários, com zeros à esquerda

        bits_lista.append(format(byte, '08b'))

    binario_final = "".join(bits_lista)

    print(f"Texto original: {texto}")
    print(f"Bytes (UTF-8): {bytes_texto}")
    print(f"Binário (UTF-8): {binario_final}")
def choice():
    choice = int(input("1-base64   2-binário"))
    ...
    if choice == 2:
        sub_choice2 = int(input("1-encode     2-decode:"))
        if sub_choice2 == 1:
            encode_bin()
